...
class CorsPreflightPredicate(object):
    ...
    def __call__(self, context, request):
        ...
        return (
            ...
            and "Origin" in request.headers  # noqa
            and "Access-Control-Request-Method" in request.headers  # noqa
        )
...
def cors_options_view(context, request):  # pylint: disable=unused-argument
    response = request.response
    if "Access-Control-Request-Headers" in request.headers:
        response.headers["Access-Control-Allow-Methods"] = "OPTIONS,HEAD,GET,POST,PUT,DELETE"
    response.headers["Access-Control-Allow-Headers"] = "Content-Type,Accept,Accept-Language,Authorization,X-Request-ID"
    return response
