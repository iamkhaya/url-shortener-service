...
def ordered_include_load(stream, Loader=yaml.Loader, object_pairs_hook=OrderedDict):
    ...
    OrderedIncludeLoader.add_constructor(yaml.resolver.BaseResolver.DEFAULT_MAPPING_TAG, construct_mapping)
...
def server_error(exc, request):
    ...
    message = getattr(exc, "message", "API Error")
    logger.info("error %s on url: %s", request.url, status)
    if status == 500:
